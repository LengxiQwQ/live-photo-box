...
internal static class NativeHeifBoxParser
{
    ...
    [DllImport(NativeMethods.LibraryName, CallingConvention = CallingConvention.Cdecl, EntryPoint = "lpb_heif_locate_xmp_item", ExactSpelling = true)]
    private static extern NativeResult lpb_heif_locate_xmp_item(
        nint context,
        byte[] input,
        nuint inputSize,
        out ulong outOffset,
    ...
    /// <summary>
    /// Try to locate the XMP item offset and length via the Native C++ parser.
    /// </summary>
    public static bool TryLocateXmpItem(byte[] data, out long offset, out long length, out string? error)
    {
        ...
        try
        {
            ...
            var result = lpb_heif_locate_xmp_item(
                context,
                data,
                (nuint)data.Length,
                out ulong cOffset,
