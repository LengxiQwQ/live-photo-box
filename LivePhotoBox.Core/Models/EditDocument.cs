...
public sealed record EditDocument
{
    ...
    /// <summary>
    /// 从外部已探测的 Native SourceMediaFacts 与基础信息适配创建文档（C# Adapter 逻辑）。
    /// 注意：本方法只做内存事实赋值映射，不触发任何文件解析或 Native 调用。
    /// </summary>
        ...
        EditMetadataFacts? metadata = null)
    {
        ...
        {
            if (!string.IsNullOrEmpty(motionPath) && File.Exists(motionPath))
