...
namespace LivePhotoBox.Services.Protocols
{
    ...
    public sealed class HuaweiMovingPhotoProtocol : LivePhotoProtocol
    {
        ...
        public static byte[] BuildTail(int coverFrame, int totalFrames, long mp4Size,
            string tailPrefix = "v6_f", bool preferNative = false)
        {
            if (preferNative)
            {
                if (Interop.NativeHuaweiMovingPhoto.TryBuildTail(
                    coverFrame, totalFrames, mp4Size, 0, 0, tailPrefix, out byte[] nativeOutput, out string? nativeError))
                {
                    LogService.Merge("HUAWEI LIVE_ tail generated by Native writer.", Models.LogLevel.Debug);
                    return nativeOutput;
                }
                LogService.Merge($"HUAWEI Native tail writer failed; using Legacy: {nativeError}", Models.LogLevel.Warning);
            }
        ...
        /// </summary>
        public static byte[] BuildTail(int coverFrame, int totalFrames, long mp4Size,
            int originalCoverMs, int originalDurationMs, string tailPrefix = "v6_f", bool preferNative = false)
        {
            if (preferNative)
            {
                if (Interop.NativeHuaweiMovingPhoto.TryBuildTail(
                    coverFrame, totalFrames, mp4Size, originalCoverMs, originalDurationMs, tailPrefix, out byte[] nativeOutput, out string? nativeError))
                {
                    LogService.Merge("HUAWEI LIVE_ tail (with history) generated by Native writer.", Models.LogLevel.Debug);
                    return nativeOutput;
                }
                LogService.Merge($"HUAWEI Native tail writer failed; using Legacy: {nativeError}", Models.LogLevel.Warning);
            }

            byte[] tail = BuildTail(coverFrame, totalFrames, mp4Size, tailPrefix, false);
