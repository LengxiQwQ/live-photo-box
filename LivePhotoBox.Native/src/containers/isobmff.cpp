...
bool validate_video_table(const uint8_t* data, size_t stbl_start, size_t stbl_end,
    ...
    // compact stz2 table (nor composition-time offsets).  Reject these
    // structures explicitly instead of treating their bytes as stsz/stts
    if (stz2.size != 0) return false;
    isobmff_box_header ctts{};
    size_t ctts_s = 0;
    if (!direct_child(data, stbl_start, stbl_end, "ctts", ctts, ctts_s)) return false;
    if (ctts.size != 0) return false;
    ...
    std::vector<media_payload_range> chunks;
    ...
    for (uint32_t chunk = 1; chunk <= chunk_count; ++chunk) {
        ...
        if (containing == mdats.end() && raw <= std::numeric_limits<uint64_t>::max() - origin) {
            absolute = raw + origin;
            containing = std::find_if(mdats.begin(), mdats.end(), [&](const media_payload_range& range) {
                return absolute >= range.start && absolute < range.end;
            });
        }
        ...
        if (chunk_end > containing->end) return false;
...
bool validate_media_range(const uint8_t* data, size_t data_size, uint64_t offset, uint64_t length, bool require_ftyp) noexcept {
    ...
    while(pos<end) {
        ...
        if(type_is(data,pos,"mdat")){
            if(box.size==box.header_size)return false;
            mdat=true;
