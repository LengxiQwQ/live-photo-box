...
static std::string extract_xmp_string(lpb_context* context, const std::vector<uint8_t>& data, lpb_image_container container) {
    ...
    if (container == LPB_IMAGE_CONTAINER_HEIC) {
        ...
        if (lpb_heif_enumerate_xmp_items(context, data.data(), data.size(),
                ranges.data(), ranges.size(), &range_count) != LPB_RESULT_OK) return {};

        std::string fallback;
        std::string motion_packet;
        size_t motion_packet_count = 0;
        for (const auto& range : ranges) {
            if (range.offset > data.size() || range.length > data.size() - static_cast<size_t>(range.offset)) return {};
            ...
            if (!scan_xmp_tree(packet, packet_nodes)) return {};
        ...
        if (motion_packet_count > 1) return {};
        return motion_packet_count == 1 ? motion_packet : fallback;
    }
    return {};
}
...
static bool extract_apple_cid_from_owned_exif(const uint8_t* data, size_t start,
    ...
    auto mark_apple_conflict_if_owned = [&]() noexcept {
        // A vendor may legally reuse Exif tag 0x927C for non-Apple data.
        // Duplicate ownership is only an Apple ambiguity when the owned
        // MakerNote actually carries Apple's formal signature. Otherwise do
        // not turn an unrelated vendor Exif layout into an Apple failure.
        if (duplicate_tag && std::string_view(reinterpret_cast<const char*>(data + start), end - start)
                .find("Apple iOS") != std::string_view::npos) {
            out_has_conflict = true;
        }
    };
    auto find_tag = [&](uint32_t ifd_offset, uint16_t wanted, uint16_t& type, uint32_t& count, uint32_t& value) noexcept {
        if (ifd_offset > end - start - 2) return false;
        const size_t pos = start + ifd_offset; uint16_t n = 0;
        if (!u16(pos, n) || n > 512 || static_cast<size_t>(n) * 12 > end - pos - 2) return false;
        bool found = false;
        for (uint16_t i = 0; i < n; ++i) {
            const size_t e = pos + 2 + static_cast<size_t>(i) * 12;
            uint16_t tag = 0, entry_type = 0;
            uint32_t entry_count = 0, entry_value = 0;
            if (!u16(e, tag) || !u16(e + 2, entry_type) || !u32(e + 4, entry_count) || !u32(e + 8, entry_value)) return false;
    ...
    uint16_t type = 0; uint32_t count = 0; uint32_t exif_offset = 0;
    if (!find_tag(ifd0, 0x8769, type, count, exif_offset)) {
        mark_apple_conflict_if_owned();
        return false;
    }
    if (type != 4 || count != 1) return false;
    uint16_t maker_type = 0; uint32_t maker_count = 0; uint32_t maker_offset = 0;
    if (!find_tag(exif_offset, 0x927C, maker_type, maker_count, maker_offset)) {
        mark_apple_conflict_if_owned();
        return false;
    }
    if (maker_type != 7 || maker_count == 0) return false;
    const size_t maker_bytes = static_cast<size_t>(maker_count);
    size_t maker_start = 0;
    if (maker_bytes <= 4) {
        // For inline values the raw value is stored in the entry; locate it
        // by re-reading the ExifIFD entry rather than treating value as offset.
        const size_t pos = start + exif_offset; uint16_t n = 0;
        if (!u16(pos, n)) return false;
        for (uint16_t i = 0; i < n; ++i) {
            const size_t e = pos + 2 + static_cast<size_t>(i) * 12; uint16_t tag = 0;
            if (!u16(e, tag)) return false;
            if (tag == 0x927C) { maker_start = e + 8; break; }
        }
    } else {
        if (maker_offset > end - start || maker_bytes > end - start - maker_offset) return false;
        maker_start = start + maker_offset;
    }
    if (maker_start == 0 || maker_bytes > end - maker_start) return false;
    return extract_apple_cid_from_makernote(data, maker_start, maker_start + maker_bytes, out, out_has_conflict);
}
    ...
    if (container == LPB_IMAGE_CONTAINER_JPEG && data.size() >= 2 && data[0] == 0xFF && data[1] == 0xD8) {
        ...
        while (p + 2 <= data.size()) {
            ...
            if (marker == 0xE1 && payload_size >= 6 && std::memcmp(data.data() + payload, "Exif\0\0", 6) == 0) {
                ...
                if (extract_apple_cid_from_owned_exif(data.data(), payload + 6, payload + payload_size, candidate, candidate_conflict)) {
                    if (!found_cid.empty()) {
                        out_has_conflict = true;
                        return false;
                    }
    ...
    std::vector<lpb_confirmed_residue>* out_residues) noexcept
{
    ...
    if (img_cont == LPB_IMAGE_CONTAINER_HEIC) {
        std::string heif_xmp = extract_xmp_string(context, primary_data, img_cont);
    ...
    if (img_cont == LPB_IMAGE_CONTAINER_JPEG && xmp_parsed && !nodes.empty()) {
        ...
        if (find_container_directory(nodes, neutral_dir) && neutral_dir.items.size() == 2) {
