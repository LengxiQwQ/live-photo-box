...
> **v1.15.2** (2026-07-02) **[新增+修复]** 新增修复选项对话框（分类勾选+数量统计）；修复非打包模式 GitHub Token 保存不持久的问题
>
> **v1.15.1** (2026-07-02) **[修复+性能]** 修复非打包模式大量底层 bug（协议选择回弹、语言切换误判、设置读写错误、WebView2 白屏）、自动更新体验优化（安装后自启、检查间隔修复、错误重试）、卸载残留清理、安装包语言裁剪；**启动时间从约 4-5 秒缩短至 1-2 秒**（硬件检测异步化 + 双检锁防并发）
>
> **v1.15.0** (2026-07-01) **[发布]** 新增自动更新（GitHub Releases 检测 + 静默安装/替换脚本）、元数据组合匹配（GPS+设备信息+iOS 版本）、仅修复 Apple 照片筛选、外部工具检测器；修复非打包模式日志路径不一致问题
