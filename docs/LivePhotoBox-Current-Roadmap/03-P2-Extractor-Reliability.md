...
> **Entry:** P1 可信  
> **Goal:** 只从 Inspector 已确认的事实中，精确提取媒体资产。

## Invariants

Extractor 不重新猜协议。
Extractor 不自己做第二套协议识别。
它只能消费：

```text
SourceFacts / confirmed ranges / confirmed relationships
```
...
- 视频可 probe/play；
- 图片可 decode；
- GainMap/Auxiliary 的边界正确；
- malformed confirmed-range 防御；
- cancel；
- disk full；
- output path failure；
- temp cleanup；
