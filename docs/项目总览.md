...
当前架构由四个一级工程组成：`LivePhotoBox.Core`（共享核心：协议、合成/拆分/修复服务、本地化）、`LivePhotoBox`（WinUI 3 主应用 GUI）、`LivePhotoBox.CLI`（命令行工具 `livephotobox`）和 `LivePhotoBox.Native`（Visual C++/C++20 原生后端）。GUI 与 CLI 经 Core 的 Interop 层通过稳定 C ABI 调用 Native。当前重构新增一个全局分支开关：默认进入尚未实现协议的新 `rebuilt` 分支并明确停止；关闭开关才使用保留的 Legacy 兼容实现。详见 `docs/LivePhotoBox-Current-Roadmap/01-协议后端分流开关与路由审计路线.md`。
...
>
> **重构状态说明（2026-09-01）**：默认 `rebuilt` 分支尚未实现任何厂商协议，不能用于产出协议文件；现有协议能力仅在用户显式切换到 `legacy` 后作为兼容备份运行。后续先实现中性媒体，再逐项重构协议。
...
- 现有 Native 协议辅助能力只属于保留的 Legacy 兼容实现；当前默认 `rebuilt` 分支不调用它们。后续先建立中性媒体能力，再决定哪些底层能力进入新分支
...
| C# 源文件 (.cs) | ~205（Core 102 + GUI 84 + CLI 19） |
| XAML 页面/控件 (.xaml) | 16 |
| Models | ~23（GUI 12 + Core 11） |
| ViewModels | 13 |
| Views (页面) | 9 |
| Controls (自定义控件) | 9 |
| Services | ~57（Core 33 + GUI 24） |
