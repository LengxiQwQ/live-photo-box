...
├── Live Photo Box.sln                  — VS 解决方案（9 个工程）
...
位于项目根目录，共 **9 个工程**：`LivePhotoBox`（GUI）、`LivePhotoBox.Core`、`LivePhotoBox.CLI`、`ThumbnailStressTest`，以及 `tests/` 下的 `LivePhotoBox.Core.Tests`、`LivePhotoBox.CLI.Tests`、`LivePhotoBox.UITests`、`LivePhotoBoxStressTest`、`LivePhotoBox.Benchmarks`。全部测试工程都在解决方案内。
...
| 指标 | 数量 |
|------|------|
| 项目 | 3 (Core + GUI + CLI)；解决方案共 9 个工程（含 6 个测试工程） |
