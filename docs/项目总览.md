...
| 功能 | 说明 |
|------|------|
| **🔗 实况照片合成 (Merge)** | 将任意静态图片 + 视频素材组合为标准实况照片。支持 `Google Micro Video (v1)` / `Google Motion Photo (v2)` / OPPO / vivo / Samsung / HUAWEI 等协议，自动写入完整 `EXIF` + `QuickTime` 元数据 |
