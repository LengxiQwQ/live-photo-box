...
│   ├── Interop/                     # Native C ABI 的 LibraryImport 与运行时健康检查
...
- 与 Core 平级的正式 Visual C++ 工程，Visual Studio 可直接浏览、编辑、构建和调试；Phase 1 使用 MSVC x64/C++20，公开接口仅为稳定 C ABI
...
- `LivePhotoBox.Core/Interop/` 使用 `LibraryImport` 加载 Native DLL并执行 ABI、版本和 capability 健康检查
...
| C# 源文件 (.cs) | ~166（Core 64 + GUI 83 + CLI 19） |
| XAML 页面/控件 (.xaml) | 16 |
