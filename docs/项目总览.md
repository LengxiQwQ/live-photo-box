...
| **💻 CLI (livephotobox)** | 与 GUI 共享 100% Core：`merge` / `split` / `repair` / `cover` / `protocols` / `update`，支持批量、`--json`、`--dry-run`、命名模板 |
...
│   ├── Program.cs                  # 入口（System.CommandLine，merge/split/repair/protocols/cover/update + --info）
│   ├── Commands/                   # MergeCommand / SplitCommand / RepairCommand / ProtocolsCommand / CoverCommand / UpdateCommand
...
| C# 源文件 (.cs) | ~155（Core 57 + GUI 79 + CLI 19） |
| XAML 页面/控件 (.xaml) | 15 |
| Models | ~22（GUI 12 + Core 10） |
| ViewModels | 13 |
| Views (页面) | 9 |
| Controls (自定义控件) | 5 |
| Services | ~49（Core 27 + GUI 22） |
