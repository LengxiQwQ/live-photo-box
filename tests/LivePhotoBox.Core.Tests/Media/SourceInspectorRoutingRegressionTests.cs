...
{
    [Fact]
    public void LightboxRouting_UsesNativeInspector_AndHasNoBasenameOrManagedScanAuthority()
    {
        string source = ReadProductionSource("LivePhotoBox", "Services", "LightboxItemSource.cs");

        Assert.Contains("SourceInspector", source, StringComparison.Ordinal);
        Assert.Contains("InspectAsync", source, StringComparison.Ordinal);
        Assert.Contains("TryConfirmNativePair", source, StringComparison.Ordinal);
        Assert.Contains("facts.PrimaryImage.IsPresent", source, StringComparison.Ordinal);
        Assert.Contains("t.SourcePath", source, StringComparison.Ordinal);
        Assert.DoesNotContain("long videoLen = t.AppendedVideoLength", source, StringComparison.Ordinal);
        Assert.DoesNotContain("FindPairedVideo(", source, StringComparison.Ordinal);
        Assert.DoesNotContain("VideoPath = string.IsNullOrWhiteSpace(t.VideoPath) ? null : t.VideoPath", source, StringComparison.Ordinal);
        ...
        Assert.DoesNotContain("HasLiveTailMarker", source, StringComparison.Ordinal);
    }
    ...
    public void CoverRouting_UsesNativeRangeAndContainerFacts_Only()
    {
        ...
        string preview = ReadProductionSource("LivePhotoBox", "Controls", "LightboxPreview.xaml.cs");
        Assert.Contains("SourceInspectionException", preview, StringComparison.Ordinal);
        Assert.Contains("SourceProtocol.NonLive", preview, StringComparison.Ordinal);
        Assert.DoesNotContain("探测失败静默", preview, StringComparison.Ordinal);
