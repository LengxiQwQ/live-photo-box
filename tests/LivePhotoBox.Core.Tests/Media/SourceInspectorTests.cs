...
public sealed class SourceInspectorTests
{
    ...
    public async Task Inspect_GoogleV1_ConflictingXmpDeclarations_FailClosed()
    {
        ...
        Assert.Equal(NativeRuntime.GoogleV1Capability, ex.Capability);
        Assert.Contains("conflicting or malformed", ex.Message, StringComparison.OrdinalIgnoreCase);
        Assert.Contains("XMP", ex.Message, StringComparison.OrdinalIgnoreCase);
    }
